...
function BgPicker({ bgColor, bgImage, onChange, onImageBg, onClearImageBg, imgBgRef }) {
  const BG_COLORS = ["#FDFAF4", "#FFFFFF", "var(--kid-teal)", "#1e293b", "#FEF9C3", "#DBEAFE", "#DCFCE7", "#FCE7F3", "#F5F0E8", "#EEF2FF", "#111827", "#7f1d1d"];
  return (
      ...
      <div className="border-t border-gray-100 pt-3">
        ...
        <div className="flex flex-col gap-2">
          <button onClick={() => imgBgRef.current?.click()}
            ...
            style={{ background: "var(--kid-teal)", fontFamily: "Cairo,sans-serif" }}>
...
export default function Whiteboard() {
  ...
  const drawDecoratedSurah = (ctx, t) => {
    ...
    ctx.fillStyle = t.color || "var(--kid-teal)";
    ctx.textAlign = "right";
    ctx.textBaseline = "top";
  ...
  const copyAyahToBoard = (text) => {
    setTextEls(prev => [...prev, { id: Date.now().toString(), x: 200 + Math.random() * 300, y: 150 + Math.random() * 200, text, color: "var(--kid-teal)", fontSize: 26 }]);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-background" style={{ background: "#F5F5F0" }}>
      ...
      <div className="flex items-center gap-1 px-2 sm:px-3 py-1.5 border-b border-border bg-white flex-shrink-0 overflow-x-auto" dir="ltr">
        ...
        <button onClick={saveImg} title="تحميل" className="p-1.5 rounded-lg hover:bg-gray-100 flex-shrink-0" style={{ color: "var(--kid-teal)" }}><Download size={18} /></button>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className={`${isMobile ? `fixed top-0 bottom-0 left-0 z-60 w-56 border-r border-border bg-card transition-transform duration-300 ${mobilePanelOpen === 'ayah' ? 'translate-x-0' : '-translate-x-full'}` : `${showAyahPanel ? 'w-52' : 'hidden'} border-l border-border bg-card flex-shrink-0`} overflow-y-auto flex flex-col`} dir="rtl"
          ...
          <div className="p-2 border-b border-border">
            <p className="text-xs font-bold mb-2" style={{ fontFamily: "Cairo,sans-serif", color: "var(--kid-teal)" }}>📖 القرآن الكريم</p>
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {panelLoading ? <div className="flex justify-center py-8"><div className="w-6 h-6 border-2 border-primary/20 border-t-primary rounded-full animate-spin" /></div> : (
              <div className="space-y-1">
                {panelSurahId !== 9 && <div className="text-xs text-center py-1" style={{ fontFamily: "Noto Naskh Arabic,serif", color: "var(--kid-teal)" }}>بِسْمِ اللَّهِ الرَّحْمَنِ الرَّحِيمِ</div>}
                {panelAyahs.map(a => (
          ...
          {textEls.map(t => {
            ...
            return (
              <div key={t.id} className="absolute" style={{ left: lx, top: ly - t.fontSize * scaleY * zoomLevel, zIndex: 6, pointerEvents: "auto" }}>
                {editingTextId === t.id ? (
                  <div className={`rounded-[24px] border-2 bg-[#fcf7eb]/95 p-4 shadow-[0_12px_30px_rgba(0,0,0,0.16)] ${t.kind === "surahPage" ? "min-w-[340px]" : "min-w-[140px]"}`} style={{ borderColor: t.kind === "surahPage" ? (t.borderColor || "#C8A96B") : "var(--kid-teal)" }}>
                    {t.kind === "surahPage" && (
                      <div className="d-flex flex-column">
                        ...
                        <p className="text-sm font-black text-center mb-3" style={{ fontFamily: "Cairo,sans-serif" , color:"var(--kid-teal)" }}>بسم الله الرحمن الرحيم</p>
            ...
            return (
              <div key={img.id} className="absolute group"
                style={{
                  ...
                  border: isSelected ? "2px solid var(--kid-teal)" : "2px solid transparent",
                  borderRadius: 4,
                ...
                <button onClick={e => { e.stopPropagation(); setImageEls(p => p.map(im => im.id === img.id ? { ...im, locked: !im.locked } : im)); }}
                  ...
                  style={{ background: img.locked ? "var(--kid-teal)" : "rgba(0,0,0,0.5)", color: "white" }}
        ...
        <div className={`${isMobile ? `fixed top-0 bottom-0 right-0 z-60 w-56 border-l border-border bg-card transition-transform duration-300 ${mobilePanelOpen === 'tools' ? 'translate-x-0' : 'translate-x-full'}` : 'w-52 border-r border-border bg-card flex-shrink-0'} overflow-y-auto`} dir="rtl" style={isMobile ? {} : { background: "#fafafa" }}>
          ...
          <div className="p-2 border-b border-border space-y-2">
            <div>
              ...
              <input type="range" min={1} max={30} value={strokeWidth} onChange={e => setStrokeWidth(+e.target.value)} className="w-full h-1" style={{ accentColor: "var(--kid-teal)" }} />
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1 font-semibold" style={{ fontFamily: "Cairo,sans-serif" }}>شفافية: <span>{opacity}</span>%</p>
              <input type="range" min={10} max={100} value={opacity} onChange={e => setOpacity(+e.target.value)} className="w-full h-1" style={{ accentColor: "var(--kid-teal)" }} />
            </div>
          </div>
